...
// Function to return FirewallRule resource by name.
func (m ResourceListFirewallrules) GetByName(search string) ResourceFirewallRule {
	for _, resource := range m.Firewallrules {
		if resource.Metadata.Name == search {
			return resource
		}
	}
	var emptyResponse ResourceFirewallRule
	return emptyResponse
}

// Function to return FirewallRule resource by uid.
func (m ResourceListFirewallrules) GetByUid(search string) ResourceFirewallRule {
	for _, res := range m.Firewallrules {
		if res.Metadata.Uid == search {
			return res
		}
	}
...
// Function to return FirewallRule resource by label.
func (m ResourceListFirewallrules) GetByLabels(search map[string]string) []ResourceFirewallRule {
	var Response []ResourceFirewallRule
	for _, res := range m.Firewallrules {
		if len(res.Metadata.Labels) != 0 {
			if stringhelper.CompareLabels(search, res.Metadata.Labels) {
				Response = append(Response, res)
